...
const Pending_Auction = () => {
  ...
  return (
    <div className="p-5">
      <h3 className="text-start w-100 mb-4 border-bottom text-primary">Pending Auction</h3>
      <div className={css["auction-table"]}>
        {rejectingAuction ? (
          <div style={{height: "500px"}}>
            ...
            <div className="mt-2">
              <button className="btn btn-link me-3" onClick={handleCancelReject}>Back</button>
              ...
              {msg ? (
                ...
                sellerdata.map((item, index) => (
                  <tr key={item._id} className="border-top mt-5">
                    ...
                    <td className="text-muted">{item.product_name}</td>
                    <td className="text-muted">{item.starting_price}</td>
                    <td className="text-muted">{item.increment_price}</td>
                    <td className="text-muted">{item.start_date}</td>
                    <td className="text-muted">{item.end_date}</td>
                    <td className="text-muted">{item.product_type}</td>
